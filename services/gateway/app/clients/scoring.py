...
async def get_company_score(company_id: str) -> httpx.Response:
    ...
    async with httpx.AsyncClient(base_url=url, timeout=10.0) as client:
        try:
            ...
            logger.error(f"Failed to connect to scoring service at {full_url}: {e}", exc_info=True)
            raise
        except httpx.TimeoutException as e:
            logger.error(f"Timeout connecting to scoring service at {full_url}: {e}", exc_info=True)
            raise
